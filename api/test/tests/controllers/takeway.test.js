...
describe("lib/controllers/takeaway", () => {
  ...
  beforeEach(async () => {
    ...
    this.meeting = (
      await client.post("/meeting", fakeMeeting({ owner_id: this.user._id }))
    ).data;
  ...
  describe("#create", () => {
    ...
    it("should create takeaway if meeting owner", async () => {
      ...
      assert.deepInclude(res.data, {
        ...takeaway,
        owner_id: this.user.user._id.toString(),
      });
  ...
  describe("#delete", () => {
    ...
    it("should delete takeaway", async () => {
      ...
      assert.deepStrictEqual(res.data, { acknowledged: true, deletedCount: 1 });
